...
//		array_splice($data , $i , 1 , $line);
		mysqli_query($sql, "UPDATE `news` SET `news_id`='$news_id',`start`='$start',`sort`='$sort',`end`='$end',`sort2`='$sort2',`time`='$time',`newstitle`='$newstitle',`input`='$input',`photo1`='$photo1',`jpeg`='$jpeg',`link`='$link',`blank`='$blank' WHERE `id` LIKE '$id' LIMIT 1");
...
//写真を削除
if($operation=='delete'){
	if(file_exists('./photo/'.$news_id.'1.jpg')){
		unlink('./photo/'.$news_id.'1.jpg');
	}
}
?>
<!doctype html>
