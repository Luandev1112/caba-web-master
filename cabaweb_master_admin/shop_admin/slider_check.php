...
//		array_splice($data , $i , 1 , $line);
		mysqli_query($sql, "UPDATE `data2` SET `id`='$id',`name`='$name',`name2`='$name2',`link1`='$link1',`link2`='$link2',`link3`='$link3',`link4`='$link4',`link5`='$link5',`link6`='$link6',`link7`='$link7',`link8`='$link8',`link9`='$link9',`link10`='$link10',`end1`='$end1',`end2`='$end2',`end3`='$end3',`end4`='$end4',`end5`='$end5',`end6`='$end6',`end7`='$end7',`end8`='$end8',`end9`='$end9',`end10`='$end10',`edittime`='$edittime' WHERE `name2` LIKE '$name2' LIMIT 1");
	} elseif ($operation == 'delete') {
//		array_splice($data , $i , 1);
		mysqli_query($sql, "DELETE FROM `data2` WHERE name2 LIKE '$name2' LIMIT 1");
	}
...
$jpeg = 's.jpg';
//保存先のファイル名
	$upload_name1 = $dir . $name2 . '_n1' . $jpeg;
	$upload_name2 = $dir . $name2 . '_n2' . $jpeg;
	$upload_name3 = $dir . $name2 . '_n3' . $jpeg;
	$upload_name4 = $dir . $name2 . '_n4' . $jpeg;
	$upload_name5 = $dir . $name2 . '_n5' . $jpeg;
	$upload_name6 = $dir . $name2 . '_n6' . $jpeg;
	$upload_name7 = $dir . $name2 . '_n7' . $jpeg;
	$upload_name8 = $dir . $name2 . '_n8' . $jpeg;
	$upload_name9 = $dir . $name2 . '_n9' . $jpeg;
	$upload_name10 = $dir . $name2 . '_n10' . $jpeg;

	// m
	$upload_name1_m = $dir . $name2 . '_n1_m' . $jpeg;
	$upload_name2_m = $dir . $name2 . '_n2_m' . $jpeg;
	$upload_name3_m = $dir . $name2 . '_n3_m' . $jpeg;
	$upload_name4_m = $dir . $name2 . '_n4_m' . $jpeg;
	$upload_name5_m = $dir . $name2 . '_n5_m' . $jpeg;
	$upload_name6_m = $dir . $name2 . '_n6_m' . $jpeg;
	$upload_name7_m = $dir . $name2 . '_n7_m' . $jpeg;
	$upload_name8_m = $dir . $name2 . '_n8_m' . $jpeg;
	$upload_name9_m = $dir . $name2 . '_n9_m' . $jpeg;
	$upload_name10_m = $dir . $name2 . '_n10_m' . $jpeg;
	...
	if ($result) {
		...
		echo "<div class=\"slider_link_area_2\" style=\"text-align: left;\">
											<p style=\"padding: 0; margin: 0;\">
												<label>
													<span>■掲載終了日</span>
													<span style=\"font-size: 80%; color: #4D4D4D;\">".$end4."</span>
												</label>                 
											</p>
										</div> 										
									</td>\n";
...
if ($photo_name1 != $empty) {$photo1 = $dir2 . $name2 . '_n1s';}
if ($photo_name2 != $empty) {$photo2 = $dir2 . $name2 . '_n2s';}
if ($photo_name3 != $empty) {$photo3 = $dir2 . $name2 . '_n3s';}
if ($photo_name4 != $empty) {$photo4 = $dir2 . $name2 . '_n4s';}
if ($photo_name5 != $empty) {$photo5 = $dir2 . $name2 . '_n5s';}
if ($photo_name6 != $empty) {$photo6 = $dir2 . $name2 . '_n6s';}
if ($photo_name7 != $empty) {$photo7 = $dir2 . $name2 . '_n7s';}
if ($photo_name8 != $empty) {$photo8 = $dir2 . $name2 . '_n8s';}
if ($photo_name9 != $empty) {$photo9 = $dir2 . $name2 . '_n9s';}
if ($photo_name10 != $empty) {$photo10 = $dir2 . $name2 . '_n10s';}
...
//写真を削除
if($delete1=='delete'){
	unlink('../photo/'.$name2.'_n1s.jpg');
	unlink('../photo/'.$name2.'_n1_ms.jpg');
	
	echo "<p>".$name."スライダー画像1枚目を削除しました。</p>\n";
}
if($delete2=='delete'){
	unlink('../photo/'.$name2.'_n2s.jpg');
	unlink('../photo/'.$name2.'_n2_ms.jpg');
		
	echo "<p>".$name."スライダー画像2枚目を削除しました。</p>\n";
}
if($delete3=='delete'){
	unlink('../photo/'.$name2.'_n3s.jpg');
	unlink('../photo/'.$name2.'_n3_ms.jpg');
		
	echo "<p>".$name."スライダー画像3枚目を削除しました。</p>\n";
}
if($delete4=='delete'){
	unlink('../photo/'.$name2.'_n4s.jpg');
	unlink('../photo/'.$name2.'_n4_ms.jpg');
		
	echo "<p>".$name."スライダー画像4枚目を削除しました。</p>\n";
}
if($delete5=='delete'){
	unlink('../photo/'.$name2.'_n5s.jpg');
	unlink('../photo/'.$name2.'_n5_ms.jpg');
		
	echo "<p>".$name."スライダー画像5枚目を削除しました。</p>\n";
}
if($delete6=='delete'){
	unlink('../photo/'.$name2.'_n6s.jpg');
	unlink('../photo/'.$name2.'_n6_ms.jpg');

	echo "<p>".$name."スライダー画像6枚目を削除しました。</p>\n";
}
if($delete7=='delete'){
	unlink('../photo/'.$name2.'_n7s.jpg');
	unlink('../photo/'.$name2.'_n7_ms.jpg');

	echo "<p>".$name."スライダー画像7枚目を削除しました。</p>\n";
}
if($delete8=='delete'){
	unlink('../photo/'.$name2.'_n8s.jpg');
	unlink('../photo/'.$name2.'_n8_ms.jpg');

	echo "<p>".$name."スライダー画像8枚目を削除しました。</p>\n";
}
if($delete9=='delete'){
	unlink('../photo/'.$name2.'_n9s.jpg');
	unlink('../photo/'.$name2.'_n9_ms.jpg');

	echo "<p>".$name."スライダー画像9枚目を削除しました。</p>\n";
}
if($delete10=='delete'){
	unlink('../photo/'.$name2.'_n10s.jpg');
	unlink('../photo/'.$name2.'_n10_ms.jpg');
