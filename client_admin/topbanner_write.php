...
if (($photo_type1 == 'image/jpeg') || ($photo_type1 == 'image/pjpeg')) {
	...
	if ($result) {
		...
		//アップロードの成功
		echo '<a class="media-left media-top" href="' . $upload_name1 . '" target="_blank"><img src="' . $upload_name1. '?' . $koshin . '" alt="photo1"  width="20%" /></a><div class="media-body"><h4 class="media-heading">トップバナー画像1枚目</h4>';
		...
		if ($result_m) {
			...
			if (imagejpeg($image_m , $upload_name1_m , $quality)) {
				...
				echo '<p>トップバナー画像保存失敗</p>';
			}
		} else {
			echo '<p>トップバナー画像作成失敗</p>';
		}
...
if($delete1=='delete'){
	...
	echo "<p>".$name."トップバナー画像1枚目を削除しました。</p>\n";
}
if($delete2=='delete'){
