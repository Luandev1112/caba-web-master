...
//		array_splice($data , $i , 1 , $line);
		mysqli_query($sql, "UPDATE `data2` SET `id`='$id',`name`='$name',`name2`='$name2',`link1`='$link1',`link2`='$link2',`link3`='$link3',`link4`='$link4',`link5`='$link5',`link6`='$link6',`link7`='$link7',`link8`='$link8',`link9`='$link9',`link10`='$link10',`edittime`='$edittime' WHERE `name2` LIKE '$name2' LIMIT 1");
...
<html lang="ja"><head>
<meta charset="utf-8">
<title>店舗スライダー画像設定｜CLIENT Admin Tool</title>
<meta name="description" content="管理画面ツール　店舗スライダー画像設定">
...
if ($photo_name1 != $empty) {$photo1 = $dir . $name2 . '_1s';}
if ($photo_name2 != $empty) {$photo2 = $dir . $name2 . '_2s';}
if ($photo_name3 != $empty) {$photo3 = $dir . $name2 . '_3s';}
if ($photo_name4 != $empty) {$photo4 = $dir . $name2 . '_4s';}
if ($photo_name5 != $empty) {$photo5 = $dir . $name2 . '_5s';}
if ($photo_name6 != $empty) {$photo6 = $dir . $name2 . '_6s';}
if ($photo_name7 != $empty) {$photo7 = $dir . $name2 . '_7s';}
if ($photo_name8 != $empty) {$photo8 = $dir . $name2 . '_8s';}
if ($photo_name9 != $empty) {$photo9 = $dir . $name2 . '_9s';}
if ($photo_name10 != $empty) {$photo10 = $dir . $name2 . '_10s';}
...
//写真を削除
if($delete1=='delete'){
	unlink('./photo/'.$name2.'_1s.jpg');
	unlink('./photo/'.$name2.'_1_ms.jpg');
	
	echo "<p>".$name."スライダー画像1枚目を削除しました。</p>\n";
}
if($delete2=='delete'){
	unlink('./photo/'.$name2.'_2s.jpg');
	unlink('./photo/'.$name2.'_2_ms.jpg');
		
	echo "<p>".$name."スライダー画像2枚目を削除しました。</p>\n";
}
if($delete3=='delete'){
	unlink('./photo/'.$name2.'_3s.jpg');
	unlink('./photo/'.$name2.'_3_ms.jpg');
		
	echo "<p>".$name."スライダー画像3枚目を削除しました。</p>\n";
}
if($delete4=='delete'){
	unlink('./photo/'.$name2.'_4s.jpg');
	unlink('./photo/'.$name2.'_4_ms.jpg');
		
	echo "<p>".$name."スライダー画像4枚目を削除しました。</p>\n";
}
if($delete5=='delete'){
	unlink('./photo/'.$name2.'_5s.jpg');
	unlink('./photo/'.$name2.'_5_ms.jpg');
		
	echo "<p>".$name."スライダー画像5枚目を削除しました。</p>\n";
}
if($delete6=='delete'){
	unlink('./photo/'.$name2.'_6s.jpg');
	unlink('./photo/'.$name2.'_6_ms.jpg');

	echo "<p>".$name."スライダー画像6枚目を削除しました。</p>\n";
}
if($delete7=='delete'){
	unlink('./photo/'.$name2.'_7s.jpg');
	unlink('./photo/'.$name2.'_7_ms.jpg');

	echo "<p>".$name."スライダー画像7枚目を削除しました。</p>\n";
}
if($delete8=='delete'){
	unlink('./photo/'.$name2.'_8s.jpg');
	unlink('./photo/'.$name2.'_8_ms.jpg');

	echo "<p>".$name."スライダー画像8枚目を削除しました。</p>\n";
}
if($delete9=='delete'){
	unlink('./photo/'.$name2.'_9s.jpg');
	unlink('./photo/'.$name2.'_9_ms.jpg');

	echo "<p>".$name."スライダー画像9枚目を削除しました。</p>\n";
}
if($delete10=='delete'){
	unlink('./photo/'.$name2.'_10s.jpg');
	unlink('./photo/'.$name2.'_10_ms.jpg');
