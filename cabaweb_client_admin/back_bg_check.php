...
<html lang="ja"><head>
<meta charset="utf-8">
<title>店舗トップ 背景画像確認｜Admin Tool</title>
<meta name="description" content="管理画面ツール　店舗トップ 背景画像確認">
	...
	if ($result) {
		...
		if ($result_m) {
			...
			if (imagejpeg($image_m , $upload_name1_m , $quality)) {
				echo '<p>店舗トップ背景画像画像保存</p>';
			} else {
				echo '<p>店舗トップ背景画像画像保存失敗</p>';
			}
		} else {
			echo '<p>店舗トップ背景画像画像作成失敗</p>';
		}
	...
	if ($result) {
		...
		if ($result_m) {
			...
			if (imagejpeg($image_m , $upload_name2_m , $quality)) {
				echo '<p>2枚目店舗トップ背景画像画像保存</p>';
			} else {
				echo '<p>2枚目店舗トップ背景画像画像保存失敗</p>';
			}
		} else {
			echo '<p>2枚目店舗トップ背景画像画像作成失敗</p>';
		}
...
if (($photo_type3 == 'image/jpeg') || ($photo_type3 == 'image/pjpeg')) {
	...
	if ($result) {
		...
		if ($width < '400') {
			echo '<p>[店舗トップ背景画像3]は横幅400px未満です。サイズ確認後最アップロードしてください！</p>';
		}
		...
		if ($result_m) {
			...
			if (imagejpeg($image_m , $upload_name3_m , $quality)) {
				echo '<p>3枚目店舗トップ背景画像画像保存</p>';
			} else {
				echo '<p>3枚目店舗トップ背景画像画像保存失敗</p>';
			}
		} else {
			echo '<p>3枚目店舗トップ背景画像画像作成失敗</p>';
		}
...
if (($photo_type4 == 'image/jpeg') || ($photo_type4 == 'image/pjpeg')) {
	...
	if ($result) {
		...
		if ($width < '400') {
			echo '<p>[店舗トップ背景画像4]は横幅400px未満です。サイズ確認後最アップロードしてください！</p>';
		}
		...
		if ($result_m) {
			...
			if (imagejpeg($image_m , $upload_name4_m , $quality)) {
				echo '<p>4枚目店舗トップ背景画像画像保存</p>';
			} else {
				echo '<p>4枚目店舗トップ背景画像画像保存失敗</p>';
			}
		} else {
			echo '<p>4枚目店舗トップ背景画像画像作成失敗</p>';
		}
...
//写真を削除
if($delete1=='delete'){
	unlink('../client_admin/photo/'.$name2.'_1b.jpg');
	unlink('../client_admin/photo/'.$name2.'_1_mb.jpg');
	
	echo "<p>".$name."スライダー画像1枚目を削除しました。</p>\n";
}
if($delete2=='delete'){
	unlink('../client_admin/photo/'.$name2.'_2b.jpg');
	unlink('../client_admin/photo/'.$name2.'_2_mb.jpg');
		
	echo "<p>".$name."スライダー画像2枚目を削除しました。</p>\n";
}
if($delete3=='delete'){
	unlink('../client_admin/photo/'.$name2.'_3b.jpg');
	unlink('../client_admin/photo/'.$name2.'_3_mb.jpg');
		
	echo "<p>".$name."スライダー画像3枚目を削除しました。</p>\n";
}
if($delete4=='delete'){
	unlink('../client_admin/photo/'.$name2.'_4b.jpg');
	unlink('../client_admin/photo/'.$name2.'_4_mb.jpg');
		
	echo "<p>".$name."スライダー画像4枚目を削除しました。</p>\n";
}
if($delete5=='delete'){
	unlink('../client_admin/photo/'.$name2.'_5b.jpg');
	unlink('../client_admin/photo/'.$name2.'_5_mb.jpg');
		
	echo "<p>".$name."スライダー画像5枚目を削除しました。</p>\n";
}
if($delete6=='delete'){
	unlink('../client_admin/photo/'.$name2.'_6b.jpg');
	unlink('../client_admin/photo/'.$name2.'_6_mb.jpg');

	echo "<p>".$name."スライダー画像6枚目を削除しました。</p>\n";
}
if($delete7=='delete'){
	unlink('../client_admin/photo/'.$name2.'_7b.jpg');
	unlink('../client_admin/photo/'.$name2.'_7_mb.jpg');

	echo "<p>".$name."スライダー画像7枚目を削除しました。</p>\n";
}
if($delete8=='delete'){
	unlink('../client_admin/photo/'.$name2.'_8b.jpg');
	unlink('../client_admin/photo/'.$name2.'_8_mb.jpg');

	echo "<p>".$name."スライダー画像8枚目を削除しました。</p>\n";
}
if($delete9=='delete'){
	unlink('../client_admin/photo/'.$name2.'_9b.jpg');
	unlink('../client_admin/photo/'.$name2.'_9_mb.jpg');

	echo "<p>".$name."スライダー画像9枚目を削除しました。</p>\n";
}
if($delete10=='delete'){
	unlink('../client_admin/photo/'.$name2.'_10b.jpg');
	unlink('../client_admin/photo/'.$name2.'_10_mb.jpg');
