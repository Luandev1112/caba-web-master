...
//保存先のディレクトリ
$dir = 'photo2/';
$dir2 = 'https://www.caba-web.com/nightgram_master_admin/photo2/';
//拡張子
...
//保存先のファイル名
	$upload_name1 = $dir . $photo_id . '_1' . $jpeg;
	// m
	$upload_name1_m = $dir . $photo_id . '_1_m' . $jpeg;
...
if (($photo_type1 == 'image/jpeg') || ($photo_type1 == 'image/pjpeg')) {
	...
	if ($result) {
		...
		if ($result_m) {
			//古い画像を削除
			if(file_exists('./photo/'.$photo_id.'_1_m.jpg')){
			unlink('./photo/'.$photo_id.'_1_m.jpg');
			}
			//サムネイル画像の保存 _m
...
//	}
//}
$data = mysqli_query($sql2, "SELECT * FROM `rss_information3` WHERE id LIKE '$id'");
while($row=mysqli_fetch_assoc($data)) {
include('parts/rss_info.php');
}


if ($photo_name1 != $empty) {$img_url = $dir2 . $photo_id. '_1.jpg';}



//削除写真のデータを空に
if($delete1=='delete'){$img_url='';}
...
//fclose($handle);
mysqli_query($sql2, "UPDATE `rss_information3` SET `img_url`='$img_url' WHERE `id` LIKE '$id' LIMIT 1");


//写真を削除
if($delete1=='delete'){
	unlink('../photo/'.$photo_id.'_1'.$jpeg.'');
	unlink('../photo/'.$photo_id.'_1_m'.$jpeg.'');
...
<meta http-equiv="refresh" content="0;URL=insta_photo2.php?search1=<?php echo $option23; ?>&search2=<?php echo $option24; ?>#<?php echo $id; ?>">
