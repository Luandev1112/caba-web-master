...
?>」</div>
						<div class="client_search">
							<form action="insta_photo.php" method="get" id="form">
								<div class="wrapper">
...
?>
									...
									<div class="element_search">
										<p>
											<label><span>店舗で選択</span>
												<select name="search3" id="lv3" class="group1" disabled="disabled">
													<option value="">--</option>
													<?php
...
//▽表示内容ここから
	if ($search3 == $no) {
		echo "<option value=\"".$no."\" class=\"p".$area2."\" selected=\"selected\" />".$s_name."</option>\n";
echo "".$toshi_name."　\n";
}else{
//▽表示内容ここから
echo "<option value=\"".$no."\" class=\"p".$area2."\">".$s_name."</option>\n";
}
...
$result=mysqli_query($sql2, "SELECT * From `rss_information` WHERE id!='' ".$search1_sql." ".$search2_sql." ".$search3_sql." ORDER by pub DESC");
$cnt=mysqli_num_rows($result);
